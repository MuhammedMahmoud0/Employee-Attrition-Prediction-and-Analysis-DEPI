...
# --- Utility Functions (Simulating the Model Pipeline) ---

# The full list of 24 features required by the updated model pipeline 
# (based on the user's new FastAPI EmployeeData class).
REQUIRED_FEATURES = [
...
def mock_predict(df: pd.DataFrame) -> np.ndarray:
    """
    Simulates the prediction logic of the external model pipeline.
    
    This function accepts all 24 features but uses a simplified rule set 
    based on the three core features (age, monthly_income, job_satisfaction) 
    since the actual full-feature 'best_model.pkl' is not available.
    
    NOTE: For the mock to work with the updated string inputs (Low, Medium, etc.), 
    we map JobSatisfaction to a numerical value internally.
    """
    df = df.copy() 
    df['Prediction_Int'] = 0  # Default to 0 (Stayed)

    # Internal mapping for mock logic (Low=1, Very High=4)
    js_map = {'Low': 1, 'Medium': 2, 'High': 3, 'Very High': 4}
    # Use lowercase column names as per the FastAPI class structure
    # NOTE: The mock_predict function currently depends on the existence of these 
    # three lowercase columns, which are provided in the input_df in model_page().
    
    # Check if the required columns for the mock logic exist before processing
    if 'job_satisfaction' in df.columns and 'age' in df.columns and 'monthly_income' in df.columns:
        df['JobSatisfaction_Num'] = df['job_satisfaction'].map(js_map).fillna(3) 

        # Rule 1: Young and Dissatisfied (High risk)
        condition_1 = (df['age'] < 30) & (df['JobSatisfaction_Num'] <= 2)
        df.loc[condition_1, 'Prediction_Int'] = 1

        # Rule 2: Older and Low Income (High risk)
        condition_2 = (df['monthly_income'] < 3500) & (df['age'] > 50)
        df.loc[condition_2, 'Prediction_Int'] = 1
    else:
        # If the required columns for mock logic are missing (e.g., in a visualization context), default to 0
        df['Prediction_Int'] = 0
...
def model_page():
    ...
    st.write(f"The model pipeline uses **all {len(REQUIRED_FEATURES)} features** shown below to generate a prediction.")
    ...
    with tab1:
        ...
        employee_id = st.number_input("Employee ID", min_value=1, value=999, step=1, help="Unique identifier for the employee.")
        st.markdown("---")
        ...
        with st.expander("Demographics and Compensation", expanded=True):
            ...
            # The logic to determine age_groups based on age slider value should be handled in a real app, 
            # but here we rely on the user selecting an appropriate category.
            with col7:
                age_groups = st.selectbox("Age Group", options=["18-25", "26-35", "36-45", "46-55", "56-65"], index=2)
            with col8:
                age_before_working = st.number_input("Age Before Working (Start of Career)", 0, 40, 22)
        ...
        if st.button("Predict Attrition", type="primary", use_container_width=True, help="Run the model prediction"):
            prediction_int = mock_predict(input_data)[0]
            ...
            if prediction_int == 1:
                ...
                st.write("This employee exhibits characteristics associated with a higher likelihood of attrition.")
            else:
    ...
    with tab2:
        ...
        st.info(f"Your uploaded CSV must contain **all {len(REQUIRED_FEATURES)} columns** for the model to work correctly.")
        ...
        if uploaded_file is not None:
            try:
                ...
                if missing_cols:
                    st.error(f"The uploaded CSV is missing the following required columns: {', '.join(missing_cols)}. Please check your file.")
                else:
                    with st.spinner("Running batch prediction..."):
                        # 1. Run the prediction pipeline with ALL required features
                        input_df = data[REQUIRED_FEATURES].copy()
                        
                        predictions_int = mock_predict(input_df)
                ...
                st.markdown("Please ensure the uploaded file is a valid CSV and all 24 required columns are present.")

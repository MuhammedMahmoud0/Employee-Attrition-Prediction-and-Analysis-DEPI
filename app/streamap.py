...
# --- Utility Functions (Internal Simulated Prediction) ---
...
def predict_attrition(df: pd.DataFrame) -> np.ndarray:
    """
    Performs a robust, self-contained prediction based on enhanced internal business rules.
    This function acts as the definitive prediction method for the application, 
    ensuring reliability without external API dependencies.
    """
    st.info("Using robust, self-contained business rules for prediction results.")

    df = df.copy() 
    df['Prediction_Int'] = 0  # Default to 0 (Stayed)

    # Internal mappings for prediction logic
...
def model_page():
    ...
    with tab1:
        ...
        if st.button("Predict Attrition", type="primary", use_container_width=True, help="Run the model prediction"):
            # Now calls the function that only uses the internal, reliable logic
